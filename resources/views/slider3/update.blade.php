                        ...
                        <div class="">
                            <div class="card card-primary">
                                <div class="card-header">
                                    <h3 class="card-title">Modifier Slider Bas</h3>
                                </div>
                                @if(Session::has('success'))
                                    <div class="alert alert-success" role="alert">{{Session::get('success') }}</div>
                                @endif
                                <form role="form" method="POST" action="{{ route('slider3.update',$slider3s->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="card-body">
                                        <div class="row">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <div class="custom-file">
                                                            <input type="file" class="custom-file-input" id="exampleInputFile" name="image" value="{{old('image')??$slider3s->image}}">
                                                            <label class="custom-file-label" for="exampleInputFile">{{old('image')??$slider3s->image}}</label>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
