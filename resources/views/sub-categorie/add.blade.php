                        ...
                        <div class="">
                            <div class="card card-primary">
                                <div class="card-header">
                                    <h3 class="card-title">Enrégistrer une sous-catégorie</h3>
                                </div>
                                @if(Session::has('success'))
                                    <div class="alert alert-success" role="alert">{{Session::get('success') }}</div>
                                @endif
                                <form role="form" method="POST" action="{{ route('sub-category.store') }}">
                                    @csrf
                                    <div class="card-body">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" style="width: 100%;" name="categorie_id">
                                                ...
                                                @foreach ($categories as $categorie)
                                                    <option value="{{ $categorie->id }}">{{ $categorie->libelle }} - {{ ($categorie->nom) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
