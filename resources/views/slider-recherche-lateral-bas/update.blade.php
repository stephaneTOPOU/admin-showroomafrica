                        ...
                        <div class="">
                            <div class="card card-primary">
                                <div class="card-header">
                                    <h3 class="card-title">Modifier Slider De Recherche Latéral Bas</h3>
                                </div>
                                @if(Session::has('success'))
                                    <div class="alert alert-success" role="alert">{{Session::get('success') }}</div>
                                @endif
                                <form role="form" method="POST" action="{{ route('sliderlb.update',$sliders->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control select2" style="width: 100%;" name="pays_id">
                                                        ...
                                                        @foreach ($pays as $pay)
                                                            <option value="{{ $pay->id }}" @if(($pay->id)==($sliders->pays_id)) selected @endif>{{ $pay->libelle }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <div class="custom-file">
                                                            ...
                                                            <label class="custom-file-label" for="exampleInputFile">{{old('image')??$sliders->image}}</label>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
