                        ...
                        <div class="">
                            <div class="card card-primary">
                                <div class="card-header">
                                    <h3 class="card-title">Ajouter horaire d'une entreprise</h3>
                                </div>
                                @if(Session::has('success'))
                                    <div class="alert alert-success" role="alert">{{Session::get('success') }}</div>
                                @endif
                                <form role="form" method="POST" action="{{ route('horaire.update',$horaires->id) }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="card-body">
                                        <div class="row">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control select2" style="width: 100%;" name="entreprise_id">
                                                        ...
                                                        @foreach ($entreprises as $entreprise)
                                                            <option value="{{ $entreprise->id }}" {{ $horaires->entreprise_id == $entreprise->id ? 'selected' : '' }}>{{ $entreprise->nom }} - {{ $entreprise->pays }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
